...
fun CaculateNumberology(name: String, date: String): CoreInformation {
    ...
    val day = separateAndSum(dateArray[0])
    val month = separateAndSum(dateArray[1])
    val year = reduceYearToSingleDigit(dateArray[2].toString())

    var Life =  masterNumberCheck(day + month + year)
    ...
    //Connection
    val Connection = Destiny - Life

    //Growth
    var Growth = masterNumberCheck(Destiny + Life)
    ...
    val  Connecting = Soul - separateAndSum(Personality)
    ...
    val RationalThinking = masterNumberCheck(last + first)
    ...
    //Day Of Birth
    val DayOfBirth = separateAndSum(getDayOfBirth(date))
    //Passion
    ...
    //Personal Year
    val PersonalYear = reduceAndSum(getDayMonth(date)) + reduceYearToSingleDigit(Calendar.getInstance().get(Calendar.YEAR).toString())
    ...
    //Phrase 4
    val Phrase4 = pharse4(getMonthYear(date))
...
fun calculatePythagoreanNonVowels(input: String): Int {
    ...
    val temp = masterNumberCheck(sum)
...
fun pharse4(array: IntArray): Int {
    var sum = array.sum()
...
fun reduceYearToSingleDigit(year: String): Int {
    val firstPart =  separateAndSum(year.substring(0, 2).toInt())
    val secondPart = separateAndSum(year.substring(2).toInt())

    return separateAndSum(firstPart+secondPart)
}
...
fun masterNumberCheck(number: Int): Int{
    var sum = number
    while (sum !in setOf(11, 22, 33)) {
        if (sum > 9) {
            val temp = sum
            sum = separateAndSum(temp)
        } else {
            break
        }
    }
